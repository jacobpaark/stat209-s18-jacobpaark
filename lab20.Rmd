...
**Answer**: There is generally an increase of price with the increase of carats. 


Next, show the same regression as a scatter plot with a smoothing line:

```{r}
qplot(carat, price, data = model) + geom_smooth(method = "lm")
```
...
**Answer**: Yes, we would be surprised because that is not within the range of the carat coefficient.

Would you be surprised if we collected another set of diamonds, in exactly the
same way as before, and found a slope estimate equal to 7500?

**Answer**: Yes, because it is not within the range of the carat coefficient.
...
**Answer**: This might be important to see if there's a significant correlation between the two. Looking at the diamond example, the slope is positive; therefore, there is a significant correlation between the two. 
...
**Answer**: The model predicts the price will be $3984.67

Describe in words the estimate of the slope in terms of the dataset.

**Answer**:As the depth increases by 1, the price will decrease by $29.65.
...
**Answer**: They contain only negative values.

Interpret this in words (your words, not just the formal definition).

**Answer**: The price of the diamond will decrease as the depth decreases. 
...
**Answer**: Yes, because they both do not contain 0.
...
**Answer**: The slopes are the amount of changes each unit gives to the baseline. 
