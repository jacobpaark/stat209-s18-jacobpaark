...
```{r}
bird_level <- group_summarize(birds, genus)
```
...
```{r}
bird_filtered <- filter(bird_level, n >= 5)
```
...
ggplot(bird_level, aes(egg_mass_median, male_mass_median)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  geom_point(data = bird_filtered, color = "red") +
  annotate("text", x = 8, y = 10, label = "Red Points are Genus Types of 5 or more Birds", size = 7)
```
