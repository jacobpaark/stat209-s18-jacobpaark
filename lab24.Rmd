...
There are 2 observations in the data. One row explains the burglary rate during the day time and the other one explains the burglary rate during the night time. The two slopes differ during the day and night time. This means that there is a more negative correlation between burglary rate and median income during the day than during the night. 
...
crimes_time_ca <- group_summarize(crimes, area_number, night_flag)
crimes_time_ca <- left_join(crimes_time_ca, ca, by = "area_number")
crimes_time_ca$assault_rate <- crimes_time_ca$assault_sum / crimes_time_ca$num_households * 1000

ggplot(crimes_time_ca, aes(median_income, assault_rate)) +
  geom_point(aes(color = factor(night_flag))) +
  geom_smooth(aes(color = factor(night_flag)), method = "lm") +
  scale_color_viridis(discrete = TRUE) +
  annotate("point", x = 65128, y = 23.677190, color = "salmon") +
  annotate("text", x = 65128, y = 22.677, label = "Loop")
```

## Thesis Statement

Assault rate is relatively consistent during daytime, but at night time, it heavily depends on the median income of the area. 

Population in Fuller Park are on a decline. This is due to the high crime and high concentration of households under the poverty line. This graph is also something that should not come to any surprise because it makes sense. Usually crime rates are low during the daytime but increase significantly at night time. The correlation between crime rates and median income is also something that was well known before. For Loop, the high number of crime can be explained by the huge influx and outflux of people going through the city for tourism or work purposes. 